...
vpi articulationBridge;
vb vis(1010,false);   
ll cnt;
// root is always  1   
void articulationBridgeFind(ll node) {
	cnt++;  
	dis[node] = cnt;  
	low[node] = cnt; 
	vis[node] = true; 
	...
	for(ll i = 0; i<graph[node].size(); i++) {
		...
		if(!vis[u]) {
			children++;  
			parent[u] = node;
			articulationBridgeFind(u);  
			low[node] = min(low[node],low[u]);
		}
		else if(vis[u]) low[node] = min(low[node],low[u]);
		if(dis[node] < low[u]) { 
			// root is vertex 1
			articulationBridge.pb({node,u});
		}
	}
}
void solve() {
  ...
  articulationBridgeFind(1);  
  cout << "Articulation Bridge : " << articulationBridge.size() << endl;  
  for(ll i = 0; i<articulationBridge.size(); i++) 
  	cout << articulationBridge[i].first << " -> " << articulationBridge[i].second << endl;  
  cout << endl; 
  return;
} 
int main()
